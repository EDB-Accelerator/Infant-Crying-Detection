# Updated by Kyunghun Lee Created by Xuewen Yao: X. Yao, M. Micheletti, M. Johnson, E. Thomaz, and K. de Barbaro, "Infant Crying Detection in Real-World Environments," in ICASSP 2022 (Accepted)

# This script loads an audio file and applies a high-pass filter to emphasize frequencies typical of infant cries,
# then constructs a mel spectrogram to visualize frequency distribution over time. The spectrogram undergoes noise
# reduction, and its power values are summed and smoothed to facilitate clear threshold-based event detection.
# The processed data is then further analyzed to consolidate close crying events and eliminate brief, isolated sounds,
# ensuring only significant crying periods are detected. This structured approach enables the effective isolation and
# characterization of infant crying sounds within a given audio sample, aiming to offer a reliable tool for studies or
# applications that require accurate crying detection.
